...
- [Building Actinium Core](#building-Actinium-core)
...
Clone the git repositories for Actinium and Gitian.
...
in the Actinium repository under 'Fetch and create inputs' to install sources which require
...
To build Actinium Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Actinium repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Actinium/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Actinium-project/Actinium
...
For example:
```bash
URL=https://github.com/thrasher-/Actinium.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit Actinium=${COMMIT} --url Actinium=${URL} ../Actinium/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Actinium=${COMMIT} --url Actinium=${URL} ../Actinium/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Actinium=${COMMIT} --url Actinium=${URL} ../Actinium/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the Actinium git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../Actinium/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/Actinium
SIGPATH=/some/root/path/Actinium-detached-sigs

./bin/gbuild --url Actinium=${BTCPATH},signature=${SIGPATH} ../Actinium/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Actinium-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Actinium-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/Actinium-osx-build.assert
```
